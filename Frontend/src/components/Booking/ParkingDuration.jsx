...
const ParkingDuration = () => {
  ...
  const validateInputs = () => {
    ...
    console.log("Validating Inputs:", { hours, minutes, hoursValue, minutesValue });
    ...
    if (isNaN(minutesValue) || minutesValue < 0 || minutesValue >= 60) {
      newErrors.minutes = 'Please enter a valid number between 0 and 59 for minutes.';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateInputs()) {
      ...
      if (!userId) {
        toast.error("User ID is missing.", { position: "top-center" });
        return;
      }

      try {
        const response = await fetch('http://localhost:5000/api/parking/parkingDuration', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ userId, hours: hoursValue, minutes: minutesValue }), // Include userId here
        });

        const data = await response.json();

        if (response.ok) {
          toast.success(data.message, { position: "top-center", autoClose: 1000 });
          navigate('/user/home/parkingDuration/billing-info', { state: { hours: hoursValue, minutes: minutesValue, userId } });
        } else {
          toast.error(data.message, { position: "top-center" });
        }
      } catch (error) {
        toast.error("Error saving parking duration. Please try again.", { position: "top-center" });
      }
    } else {
      toast.error("Please correct the input fields.", { position: "top-center" });
    }
  };

  return (
    <div className="flex flex-col items-center justify-center h-screen w-full bg-white p-6">
      <h1 className="text-5xl font-bold mb-8 text-gray-800 text-center">Set Parking Duration</h1>
      <form onSubmit={handleSubmit} className="flex flex-col items-center">
        <div className="flex items-center space-x-6 bg-gray-100 py-6 px-10 rounded-lg shadow-lg">
          <div className="flex flex-col items-center">
            <label htmlFor="hours" className="text-gray-500 text-lg mb-2">Hours</label>
            <input
              type="number"
              id="hours"
              placeholder="00"
          ...
          <div className="flex flex-col items-center">
            <label htmlFor="minutes" className="text-gray-500 text-lg mb-2">Minutes</label>
            <input
              type="number"
              id="minutes"
              placeholder="00"
              ...
              required
            />
            {errors.minutes && <p className="text-red-500 text-sm mt-1">{errors.minutes}</p>}
          </div>
        </div>
        <button
          type="submit"
